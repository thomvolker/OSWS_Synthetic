...
_Hint:_ When creating synthetic data, a single iteration is sufficient when there is no missing data, so we can set `maxit = 1`. Use `seed = 123` if you want to reproduce our results.
...
_Hint:_ A single iteration is again sufficient. Use `seed = 123` if you want to reproduce our results.
