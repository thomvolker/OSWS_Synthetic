...
In this workshop, you will learn how to create and evaluate synthetic data in `R`. In the practical, we will work with the `R` package `synthpop` [@synthpop]. The `synthpop` package is a powerful tool explicitly designed to generate synthetic data. Other alternatives to create synthetic data are, for example, the R-package `mice` [@mice], or the stand-alone software `IVEware` [@iveware].
...
In this workshop, you will (at least) use the packages `synthpop` [@synthpop], `ggplot2` [@ggplot2], `patchwork` [@patchwork], `psych` [@psych] and `purrr` [@purrr]. Make sure to load them (in case you haven't installed them already, install them first, using `install.packages("package.name")`).

---

__1. Install the `R`-packages `synthpop`, `ggplot2`, `patchwork`, `psych`, `tidyverse`, `purrr` from `CRAN`, and load these packages using `library()`.__
...
__4. Use `synthpop()` to create a synthetic data set in an object called `syn_param` using `method = "parametric", and set the argument `default.method = c("norm", "logreg", "polyreg", "polr")`.__

---

_Hint:_ Use `seed = 1` if you want to reproduce our results.
...
__6. Use `synthpop()` to create a synthetic data set in an object called `syn_nonparam` using `method = "cart".__
